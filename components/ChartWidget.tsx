...
//   userId: string; // 'all' or specific telegram_id
//   game: string;
//   stake: string;
//   opponent: string;
// }
...
// // User mapping for clean display names
...
//   // 1. Fetch unique users in the database to populate the User dropdown
//   useEffect(() => {
//     const fetchUsers = async () => {
//       const { data } = await supabase.from("sessions").select("telegram_id");
//       if (data) {
//         const uniqueIds = Array.from(new Set(data.map(d => d.telegram_id)));
//         const formattedUsers = uniqueIds.map(id => ({
//           id,
//           name: USER_NAME_MAP[id] || `User ${id.slice(0, 4)}`
//         }));
...
//   // 2. Fetch and process chart data
//   useEffect(() => {
...
//       // Apply User Filter (if 'all', fetches everyone's public stats)
...
//       {/* 1. COMPACT 4-COLUMN FILTER ROW */}
//       <div className="grid grid-cols-4 gap-1 px-2 pt-2.5 pb-2 bg-zinc-50/50 dark:bg-zinc-850/50 border-b border-zinc-100 dark:border-zinc-800">
        
//         <select value={filters.userId} onChange={(e) => setFilters({ ...filters, userId: e.target.value })} className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-750 rounded-lg py-1 px-1.5 text-[10px] font-bold outline-none text-zinc-600 dark:text-zinc-300">
//           <option value="all">All Users</option>
//           {usersList.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
//         </select>

//         <select value={filters.game} onChange={(e) => setFilters({ ...filters, game: e.target.value })} className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-750 rounded-lg py-1 px-1.5 text-[10px] font-bold outline-none text-zinc-600 dark:text-zinc-300">
...
//         <select value={filters.stake} onChange={(e) => setFilters({ ...filters, stake: e.target.value })} className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-750 rounded-lg py-1 px-1.5 text-[10px] font-bold outline-none text-zinc-600 dark:text-zinc-300">
//           <option value="all">All Stakes</option>
//           {[0.25, 1, 2, 5, 10, 25, 50].map(s => <option key={s} value={s}>{s}pp</option>)}
//         </select>
        
//         <select value={filters.opponent} onChange={(e) => setFilters({ ...filters, opponent: e.target.value })} className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-750 rounded-lg py-1 px-1.5 text-[10px] font-bold outline-none text-zinc-600 dark:text-zinc-300">
//           <option value="all">All Opps</option>
//           {opponentsList.map(opp => <option key={opp} value={opp}>{opp}</option>)}
//         </select>
//       </div>

//       {/* 2. MERGED TABS & TOGGLES ROW */}
...
const selectCls = "bg-transparent rounded-lg py-1.5 px-2 text-[11px] font-semibold outline-none";
const selectStyle = { backgroundColor: "var(--surface-2)", border: "1px solid var(--surface-3)", color: "var(--text-secondary)" };
...
export default function ChartWidget({ filters, setFilters, refreshTrigger }: Props) {
  ...
  useEffect(() => {
    ...
    fetchChartData();
  }, [activeTab, timePeriod, filters, refreshTrigger]);
  ...
  return (
    ...
    >
      {/* 1. FILTER ROW */}
      <div
        className="grid grid-cols-4 gap-1.5 px-3 pt-3 pb-2.5"
        style={{ borderBottom: "1px solid var(--surface-3)" }}
      >
        <select value={filters.userId} onChange={(e) => setFilters({ ...filters, userId: e.target.value })} className={selectCls} style={selectStyle}>
          <option value="all">All users</option>
          {usersList.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
        </select>

        <select value={filters.game} onChange={(e) => setFilters({ ...filters, game: e.target.value })} className={selectCls} style={selectStyle}>
          <option value="all">All games</option>
          <option value="reg">REG</option>
          <option value="prog">PROG</option>
          <option value="2-7">2-7</option>
        ...
        <select value={filters.stake} onChange={(e) => setFilters({ ...filters, stake: e.target.value })} className={selectCls} style={selectStyle}>
          <option value="all">All stakes</option>
          {[0.25, 1, 2, 5, 10, 25, 50].map(s => <option key={s} value={s}>{s}pp</option>)}
        </select>

        <select value={filters.opponent} onChange={(e) => setFilters({ ...filters, opponent: e.target.value })} className={selectCls} style={selectStyle}>
          <option value="all">All opps</option>
          {opponentsList.map(opp => <option key={opp} value={opp}>{opp}</option>)}
        </select>
      </div>
      ...
      {/* 2. TABS & TOGGLES ROW */}
      <div className="flex items-center justify-between px-3 py-2.5" style={{ borderBottom: "1px solid var(--surface-3)" }}>
        <div className="flex p-0.5 rounded-lg" style={{ backgroundColor: "var(--surface-2)" }}>
          <button
        ...
        <div className="flex items-center">
          {activeTab === "time" ? (
            ...
            <div className="flex p-0.5 rounded-lg" style={{ backgroundColor: "var(--surface-2)" }}>
              <button
                onClick={() => setHandsMetric("profit")}
                className="px-3 py-0.5 text-[10px] font-semibold rounded-md transition-all"
                style={handsMetric === "profit"
                  ? { backgroundColor: "var(--positive)", color: "#06120c" }
                  : { color: "var(--text-tertiary)" }}
              >
                $
              </button>
              <button
                onClick={() => setHandsMetric("points")}
                className="px-3 py-0.5 text-[10px] font-semibold rounded-md transition-all"
                style={handsMetric === "points"
                  ? { backgroundColor: "var(--neutral-accent)", color: "#fff" }
                  : { color: "var(--text-tertiary)" }}
              >
                Pts
              </button>
      ...
      <div className="p-3 pt-4">
        <div className="h-52 w-full text-[10px]">
          <ResponsiveContainer width="100%" height="100%">
            {activeTab === "time" ? (
              <ComposedChart data={chartData} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#8b929920" />
                <XAxis dataKey="date" tick={{ fill: '#5f6570', fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis yAxisId="left" tick={{ fill: '#5f6570', fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis yAxisId="right" orientation="right" hide />
                <Tooltip
                  contentStyle={{
                    borderRadius: '10px',
                ...
                />
                <Bar yAxisId="right" dataKey="volume" fill="#4f7fff" opacity={0.25} radius={[2, 2, 0, 0]} />
                <Line yAxisId="left" type="monotone" dataKey="profit" stroke="#3ed598" strokeWidth={2.5} dot={false} />
              </ComposedChart>
            ) : (
              <LineChart data={chartData} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#8b929920" />
                <XAxis dataKey="hands" type="number" domain={['dataMin', 'dataMax']} tick={{ fill: '#5f6570', fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: '#5f6570', fontSize: 10 }} axisLine={false} tickLine={false} />
                <Tooltip
                  contentStyle={{
                    borderRadius: '10px',
                    backgroundColor: '#1a1d22',
                  ...
                  labelStyle={{ color: '#9aa1ab' }}
                  labelFormatter={(label) => `Hands: ${label}`}
                />
                <Line
                  type="monotone"
                  dataKey={handsMetric}
                  stroke={handsMetric === "profit" ? "#3ed598" : "#a855f7"}
                  strokeWidth={2.5}
                  dot={false}
                />
              </LineChart>
